...
screenfetch
export PRINTER=hcmcp3a01
export LPDEST=hcmcp3a01
...
#}}}
## EXPORTS {{{
  export PATH=/usr/local/bin:$PATH:/home/tduong/.gem/ruby/2.0.0/bin
  #Ruby support
  if which ruby &>/dev/null; then
    GEM_DIR=$(ruby -rubygems -e 'puts Gem.user_dir')/bin
    if [[ -d "$GEM_DIR" ]]; then
      export PATH=$GEM_DIR:$PATH:/home/tduong/bin
    fi
  fi
  ## EDITOR #{{{
...
export KERN_DIR
export TOOLS_DIR=/tools/flexlm:/tools/p4v/bin
export PATH=$PATH:/opt/dropbox/:/opt/cisco/vpn/bin:/opt/cisco/anyconnect/bin:$TOOLS_DIR
if [ -f ~/.aliases ]; then
        . ~/.aliases
fi 
if [ -f ~/.bashrc_tduong ]; then
	. ~/.bashrc_tduong
fi
#[[ -z $DISPLAY && $XDG_VTNR -eq 1 ]] && exec startx
if [ -z "$DISPLAY" ] && [ -b "$XDG_VTNR" ] && [ "$XDG_VTNR" -eq 1 ]; then
		exec startx
	fi
